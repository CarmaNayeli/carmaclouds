...
(() => {
  ...
  var require_client = __commonJS({
    "../core/src/supabase/client.js"(exports, module) {
      ...
      var SupabaseTokenManager2 = class {
        constructor() {
      ...
      if (typeof window !== "undefined") {
        window.SupabaseTokenManager = SupabaseTokenManager2;
      } else if (typeof self !== "undefined") {
        self.SupabaseTokenManager = SupabaseTokenManager2;
      }
      if (typeof module !== "undefined" && module.exports) {
        module.exports = SupabaseTokenManager2;
      }
    }
  });
...
`;
  ...
  (function() {
    ...
    const HalflingLuck = {
      ...
      onRoll: function(rollResult, rollType, rollName2) {
        debug.log(`\u{1F9EC} Halfling Luck onRoll called with: ${rollResult}, ${rollType}, ${rollName2}`);
    ...
    const LuckyFeat = {
      ...
      onRoll: function(rollResult, rollType, rollName2) {
        debug.log(`\u{1F396}\uFE0F Lucky feat onRoll called with: ${rollResult}, ${rollType}, ${rollName2}`);
        const numericRollResult = parseInt(rollResult);
    ...
    const ElvenAccuracy = {
      ...
      onRoll: function(rollResult, rollType, rollName2) {
        debug.log(`\u{1F9DD} Elven Accuracy onRoll called with: ${rollResult}, ${rollType}, ${rollName2}`);
    ...
    const DwarvenResilience = {
      ...
      onRoll: function(rollResult, rollType, rollName2) {
        debug.log(`\u26CF\uFE0F Dwarven Resilience onRoll called with: ${rollResult}, ${rollType}, ${rollName2}`);
        const lowerRollName = rollName2.toLowerCase();
    ...
    const GnomeCunning = {
      ...
      onRoll: function(rollResult, rollType, rollName2) {
        debug.log(`\u{1F3A9} Gnome Cunning onRoll called with: ${rollResult}, ${rollType}, ${rollName2}`);
        const lowerRollName = rollName2.toLowerCase();
    ...
    const ReliableTalent = {
      ...
      onRoll: function(rollResult, rollType, rollName2) {
        debug.log(`\u{1F3AF} Reliable Talent onRoll called with: ${rollResult}, ${rollType}, ${rollName2}`);
        const numericRollResult = parseInt(rollResult);
    ...
    const JackOfAllTrades = {
      ...
      onRoll: function(rollResult, rollType, rollName2) {
        debug.log(`\u{1F3B5} Jack of All Trades onRoll called with: ${rollResult}, ${rollType}, ${rollName2}`);
    ...
    const RageDamageBonus = {
      ...
      onRoll: function(rollResult, rollType, rollName2) {
        debug.log(`\u{1F621} Rage Damage onRoll called with: ${rollResult}, ${rollType}, ${rollName2}`);
    ...
    const BrutalCritical = {
      ...
      onRoll: function(rollResult, rollType, rollName2) {
        debug.log(`\u{1F4A5} Brutal Critical onRoll called with: ${rollResult}, ${rollType}, ${rollName2}`);
        const numericRollResult = parseInt(rollResult);
    ...
    const BardicInspiration = {
      ...
      onRoll: function(rollResult, rollType, rollName2) {
        debug.log(`\u{1F3B5} Bardic Inspiration onRoll called with: ${rollResult}, ${rollType}, ${rollName2}`);
        if (rollType && rollType.includes("d20")) {
          debug.log(`\u{1F3B5} Bardic Inspiration: Checking if we should offer inspiration for ${rollName2}`);
    ...
    function checkRacialTraits(rollResult, rollType, rollName2) {
      debug.log(`\u{1F9EC} Checking racial traits for roll: ${rollResult} (${rollType}) - ${rollName2}`);
      ...
      for (const trait of activeRacialTraits) {
        if (trait.onRoll && typeof trait.onRoll === "function") {
          const result = trait.onRoll(rollResult, rollType, rollName2);
          if (result) {
            traitTriggered = true;
    ...
    function checkFeatTraits(rollResult, rollType, rollName2) {
      debug.log(`\u{1F396}\uFE0F Checking feat traits for roll: ${rollResult} (${rollType}) - ${rollName2}`);
  ...
  (function() {
    ...
    async function setActiveCharacter2(characterId) {
      if (typeof browserAPI === "undefined") {
        debug.warn("\u26A0\uFE0F browserAPI not available");
        return;
      }
    ...
    globalThis.setActiveCharacter = setActiveCharacter2;
  ...
  (function() {
    ...
    function applyEffectModifiers(rollName2, formula) {
      const rollLower = rollName2.toLowerCase();
      ...
      debug.log(`\u{1F3B2} Checking effects for roll: ${rollName2}`, allEffects);
      for (const effect of allEffects) {
        ...
        if (applied) {
          debug.log(`\u2705 Applied ${effect.name} (${effect.type}) to ${rollName2}`);
        }
      }
      return { modifiedFormula, effectNotes };
    }
    function checkOptionalEffects(rollName2, formula, onApply) {
      const rollLower = rollName2.toLowerCase();
      ...
      if (optionalEffects.length === 0)
        return;
      debug.log(`\u{1F3B2} Checking optional effects for roll: ${rollName2}`, optionalEffects);
      ...
      if (applicableEffects.length > 0) {
        showOptionalEffectPopup(applicableEffects, rollName2, formula, onApply);
      }
    }
    function showOptionalEffectPopup(effects, rollName2, formula, onApply) {
  ...
  `).join("");
    ...
    <p style="margin: 0 0 16px 0; color: ${colors.text};">
      You can apply an optional effect to your <strong>${rollName2}</strong> roll:
    </p>
    ${effectsList}
  ...
  `;
    ...
    function executeRoll(name, formula, effectNotes, prerolledResult = null) {
      const colorBanner = getColoredBanner(characterData);
      let rollName2 = `${colorBanner}${characterData.name} rolls ${name}`;
      if (effectNotes.length > 0) {
        rollName2 += ` ${effectNotes.join(" ")}`;
      }
      if (characterData) {
  ...
  var storage = {
    async get(keys) {
      return new Promise((resolve, reject) => {
        try {
          const result = browserAPI3.storage.local.get(keys, (items) => {
            if (browserAPI3.runtime.lastError) {
              reject(new Error(browserAPI3.runtime.lastError.message));
            } else {
              resolve(items);
            }
          });
    ...
    async set(items) {
      return new Promise((resolve, reject) => {
        try {
          const result = browserAPI3.storage.local.set(items, () => {
            if (browserAPI3.runtime.lastError) {
              reject(new Error(browserAPI3.runtime.lastError.message));
            } else {
              resolve();
    ...
    async remove(keys) {
      return new Promise((resolve, reject) => {
        try {
          const result = browserAPI3.storage.local.remove(keys, () => {
            if (browserAPI3.runtime.lastError) {
              reject(new Error(browserAPI3.runtime.lastError.message));
            } else {
              resolve();
  ...
  if (browserAPI3 && browserAPI3.storage && browserAPI3.storage.onChanged) {
    browserAPI3.storage.onChanged.addListener((changes, area) => {
      if (area !== "local")
        return;
  ...
  if (browserAPI3.alarms) {
    browserAPI3.alarms.onAlarm.addListener(async (alarm) => {
      if (alarm.name === REALTIME_KEEPALIVE_ALARM) {
        debug.log("\u23F0 Realtime keep-alive alarm triggered");
        const settings = await browserAPI3.storage.local.get(["discordWebhookEnabled", "discordPairingId", "discordWebhookUrl"]);
    ...
    browserAPI3.alarms.onAlarm.addListener((alarm) => {
  ...
  function startRealtimeKeepAlive() {
    if (browserAPI3.alarms) {
      browserAPI3.alarms.create(REALTIME_KEEPALIVE_ALARM, { periodInMinutes: 1 });
      debug.log("\u23F0 Started realtime keep-alive alarm");
    }
  }
  function stopRealtimeKeepAlive() {
    if (browserAPI3.alarms) {
      browserAPI3.alarms.clear(REALTIME_KEEPALIVE_ALARM);
  ...
  if (isFirefox) {
    ...
    try {
      const manifest = browserAPI3.runtime.getManifest();
      debug.log("\u2705 Firefox runtime accessible, version:", manifest.version);
    } catch (error) {
      debug.error("\u274C Firefox runtime not accessible:", error);
    }
    try {
      browserAPI3.storage.local.get(["test"], (result) => {
        if (browserAPI3.runtime.lastError) {
          debug.error("\u274C Firefox storage error:", browserAPI3.runtime.lastError);
        } else {
  ...
  browserAPI3.runtime.onMessage.addListener((request, sender, sendResponse) => {
        ...
        switch (request.action) {
          case "storeCharacterData":
            ...
            try {
              if (typeof SupabaseTokenManager !== "undefined") {
                const webhookSettings = await browserAPI3.storage.local.get(["discordWebhookEnabled", "discordUserId"]);
          ...
          case "extractAuthToken": {
            if (request.tabId) {
              ...
              try {
                const contentResponse = await browserAPI3.tabs.sendMessage(request.tabId, {
          ...
          case "getDiscordWebhook": {
            ...
            if (settings.enabled && settings.webhookUrl) {
              const stored = await browserAPI3.storage.local.get(["discordPairingId"]);
          ...
          case "getRealtimeStatus": {
            ...
            const settings = await browserAPI3.storage.local.get(["discordPairingId", "discordWebhookEnabled"]);
            response = {
              success: true,
              realtimeConnected,
  ...
  async function clearExistingAuthData() {
    ...
    try {
      await new Promise((resolve, reject) => {
        ...
        try {
          const result = browserAPI3.storage.local.remove(keysToRemove);
          if (result && typeof result.then === "function") {
            result.then(resolve).catch(reject);
          } else {
            browserAPI3.runtime.lastError ? reject(new Error(browserAPI3.runtime.lastError.message)) : resolve();
          }
        } catch (error) {
          reject(error);
        }
  ...
  async function loginToDiceCloud(username, password) {
    try {
      ...
      await new Promise((resolve, reject) => {
        ...
        try {
          const result = browserAPI3.storage.local.set(storageData);
          if (result && typeof result.then === "function") {
            result.then(resolve).catch(reject);
          } else {
            browserAPI3.runtime.lastError ? reject(new Error(browserAPI3.runtime.lastError.message)) : resolve();
  ...
  async function setApiToken(token, userId = null, tokenExpires = null, username = null) {
    try {
      ...
      const verification = await new Promise((resolve, reject) => {
        try {
          const result = browserAPI3.storage.local.get(["diceCloudToken", "diceCloudUserId", "tokenExpires", "username"]);
          if (result && typeof result.then === "function") {
            result.then(resolve).catch(reject);
          } else {
            browserAPI3.runtime.lastError ? reject(new Error(browserAPI3.runtime.lastError.message)) : resolve(result);
          }
        } catch (error) {
          reject(error);
        }
  ...
  async function getApiToken() {
    try {
      const result = await browserAPI3.storage.local.get(["diceCloudToken", "tokenExpires"]);
  ...
  async function checkLoginStatus() {
    try {
      const result = await browserAPI3.storage.local.get(["diceCloudToken", "username", "tokenExpires", "diceCloudUserId"]);
  ...
  async function logout() {
    try {
      ...
      const currentState = await new Promise((resolve, reject) => {
        try {
          const result = browserAPI3.storage.local.get(["diceCloudToken", "diceCloudUserId", "tokenExpires", "username", "explicitlyLoggedOut"]);
          if (result && typeof result.then === "function") {
            result.then(resolve).catch(reject);
          } else {
            browserAPI3.runtime.lastError ? reject(new Error(browserAPI3.runtime.lastError.message)) : resolve(result);
      ...
      await new Promise((resolve, reject) => {
        ...
        try {
          const result = browserAPI3.storage.local.set({ explicitlyLoggedOut: true });
          if (result && typeof result.then === "function") {
            result.then(resolve).catch(reject);
          } else {
            browserAPI3.runtime.lastError ? reject(new Error(browserAPI3.runtime.lastError.message)) : resolve();
      ...
      await new Promise((resolve, reject) => {
        ...
        try {
          const result = browserAPI3.storage.local.remove(["diceCloudToken", "diceCloudUserId", "tokenExpires", "username"]);
          if (result && typeof result.then === "function") {
            result.then(resolve).catch(reject);
          } else {
            browserAPI3.runtime.lastError ? reject(new Error(browserAPI3.runtime.lastError.message)) : resolve();
  ...
  async function storeCharacterData(characterData2, slotId) {
    try {
      if (characterData2.character_name && !characterData2.name) {
        characterData2.name = characterData2.character_name;
      }
      if (!characterData2.ownerUserId) {
        try {
          const stored = await browserAPI3.storage.local.get(["diceCloudUserId"]);
          if (stored.diceCloudUserId) {
            characterData2.ownerUserId = stored.diceCloudUserId;
          }
        } catch (e) {
          debug.warn("Could not retrieve DiceCloud user ID for ownership tagging:", e);
        }
      }
      const storageId = slotId || characterData2.id || characterData2._id || "default";
      try {
        ...
        let candidate = null;
        if (characterData2 && characterData2.raw_dicecloud_data)
          candidate = characterData2.raw_dicecloud_data;
        else if (characterData2 && characterData2._fullData && characterData2._fullData.raw_dicecloud_data)
          candidate = characterData2._fullData.raw_dicecloud_data;
        else if (characterData2 && characterData2._fullData)
          candidate = characterData2._fullData;
        ...
        if (typeof acFromRaw === "number") {
          characterData2.armorClass = acFromRaw;
        } else if (!characterData2.armorClass && characterData2.ac && typeof characterData2.ac === "number") {
          characterData2.armorClass = characterData2.ac;
        }
      } catch (e) {
        debug.warn("\u26A0\uFE0F Failed to derive armorClass for local save:", e && e.message ? e.message : e);
      }
      const result = await browserAPI3.storage.local.get(["characterProfiles", "activeCharacterId"]);
      const characterProfiles = result.characterProfiles || {};
      const characterId = characterData2.id || characterData2._id;
      if (characterId) {
        for (const [existingSlotId, existingProfile2] of Object.entries(characterProfiles)) {
          ...
          if (existingId === characterId) {
            debug.log(`\u{1F504} Deduplicating character: "${characterData2.name}" found in ${existingSlotId}, removing before saving to ${storageId}`);
      ...
      if (existingProfile && existingProfile.notificationColor) {
        if (!characterData2.notificationColor || characterData2.notificationColor === "#3498db") {
          debug.log(`\u{1F3A8} Preserving existing notification color: ${existingProfile.notificationColor}`);
          characterData2.notificationColor = existingProfile.notificationColor;
        }
      }
      characterProfiles[storageId] = characterData2;
      const updates = {
        characterProfiles,
        timestamp: Date.now()
      };
      ...
      await browserAPI3.storage.local.set(updates);
      debug.log(`Character data stored successfully for ID: ${storageId}`, characterData2);
    } catch (error) {
  ...
  async function getCharacterData(characterId = null) {
    try {
      const result = await browserAPI3.storage.local.get(["characterProfiles", "activeCharacterId", "characterData"]);
      if (result.characterData && !result.characterProfiles) {
        ...
        await browserAPI3.storage.local.remove("characterData");
        return result.characterData;
      }
      const characterProfiles = result.characterProfiles || {};
      const extractFullData = (characterData2) => {
        if (!characterData2)
          return null;
        if (characterData2.source === "database" && characterData2._fullData) {
          const fullData = characterData2._fullData.raw_dicecloud_data;
          if (fullData && typeof fullData === "object") {
            debug.log("\u{1F4E6} Extracting full character data from database raw_dicecloud_data");
            return {
              ...fullData,
              source: "database",
              lastUpdated: characterData2.lastUpdated || characterData2._fullData.updated_at,
              notificationColor: characterData2._fullData.notification_color || fullData.notificationColor
            };
          } else {
      ...
      if (characterId) {
        const characterData2 = characterProfiles[characterId] || null;
        return extractFullData(characterData2);
      }
  ...
  async function getAllCharacterProfiles() {
    try {
      ...
      const localResult = await browserAPI3.storage.local.get(["characterProfiles"]);
      ...
      try {
        if (typeof SupabaseTokenManager !== "undefined") {
          ...
          try {
            const stored = await browserAPI3.storage.local.get(["diceCloudToken", "diceCloudUserId", "tokenExpires"]);
  ...
  async function getCharacterDataFromDatabase(characterId) {
    try {
      ...
      try {
        try {
          const stored = await browserAPI3.storage.local.get(["diceCloudUserId", "diceCloudToken", "tokenExpires"]);
      ...
      debug.log("\u2705 Loaded character from database (individual fields):", characterData2.name);
      return characterData2;
    } catch (error) {
  ...
  async function setActiveCharacter(characterId) {
    try {
      const result = await browserAPI3.storage.local.get(["characterProfiles"]);
      const characterProfiles = result.characterProfiles || {};
      const characterData2 = characterProfiles[characterId];
      debug.log(`\u{1F3AF} Setting active character: ${characterId}`);
      debug.log(`\u{1F3AF} Character data:`, characterData2 ? {
        name: characterData2.name || characterData2.character_name,
        class: characterData2.class,
        level: characterData2.level
      } : "NOT FOUND");
      await browserAPI3.storage.local.set({
        activeCharacterId: characterId
      });
      debug.log(`\u2705 Active character set to: ${characterId}`);
      if (characterData2 && characterData2.id) {
        try {
          await markCharacterActiveInSupabase(characterData2.id, characterData2.name || characterData2.character_name);
        } catch (error) {
          debug.warn("\u26A0\uFE0F Failed to mark character as active in Supabase:", error);
        }
      }
      try {
        const tabs = await browserAPI3.tabs.query({ url: "https://www.owlbear.rodeo/*" });
        for (const tab of tabs) {
          await browserAPI3.tabs.sendMessage(tab.id, {
            action: "characterSelected",
            character: characterData2
          });
  ...
  async function getActiveCharacter() {
    try {
      ...
      const characterProfiles = result.characterProfiles || {};
      const characterData2 = characterProfiles[activeCharacterId];
      if (!characterData2) {
        debug.warn(`\u26A0\uFE0F Active character ${activeCharacterId} not found in profiles`);
        return null;
      }
      debug.log(`\u{1F3AF} Retrieved active character: ${characterData2.name || characterData2.character_name}`);
      return characterData2;
    } catch (error) {
  ...
  async function clearCharacterData(characterId = null) {
    try {
      if (characterId) {
        ...
        await browserAPI3.storage.local.remove(legacyKeys);
  ...
  browserAPI3.runtime.onInstalled.addListener((details) => {
    if (details.reason === "install") {
      ...
      try {
        browserAPI3.storage.local.set({ requireDiscordResync: true });
        debug.log("Set requireDiscordResync flag after install");
      } catch (e) {
        debug.warn("Could not set requireDiscordResync flag:", e);
      }
    } else if (details.reason === "update") {
      debug.log("Extension updated to version", browserAPI3.runtime.getManifest().version);
      try {
        browserAPI3.storage.local.set({ requireDiscordResync: true });
      ...
      (async () => {
        try {
          const stored = await browserAPI3.storage.local.get(["discordWebhookEnabled", "discordUserId", "discordPairingId"]);
  ...
  async function openExtensionPopup() {
    try {
      ...
      if (!isFirefox) {
        browserAPI3.runtime.openOptionsPage();
  ...
  async function setDiscordWebhookSettings(webhookUrl, enabled = true, serverName = null) {
    try {
      ...
      await browserAPI3.storage.local.set(settings);
      const verification = await browserAPI3.storage.local.get(["discordWebhookUrl", "discordWebhookEnabled"]);
  ...
  async function getDiscordWebhookSettings() {
    try {
      const result = await browserAPI3.storage.local.get([
        "discordWebhookUrl",
  ...
  async function storeCharacterToCloud(characterData2, pairingCode = null) {
    if (!isSupabaseConfigured()) {
    ...
    try {
      debug.log("\u{1F3AD} Storing character in Supabase:", characterData2.name || characterData2.id);
      const screenDimensions = typeof screen !== "undefined" && screen ? `${screen.width}x${screen.height}` : "0x0";
      const browserFingerprint = [
        navigator.userAgent,
        navigator.language,
      ...
      const visitorId = "user_" + Math.abs(hash).toString(36);
      let dicecloudUserId = characterData2.dicecloudUserId || characterData2.userId || characterData2.ownerUserId || null;
      if (!dicecloudUserId) {
        const stored = await browserAPI3.storage.local.get(["diceCloudUserId"]);
      ...
      let resolvedCharId = characterData2.id || characterData2._id || null;
      ...
      if (resolvedCharId && isStorageKey(resolvedCharId)) {
        ...
        const nested = characterData2.raw_dicecloud_data || characterData2._fullData || {};
        const possibleIds = [
          nested.dicecloud_character_id,
          nested._id,
          nested.id,
          characterData2.dicecloud_character_id,
          characterData2.dicecloudId,
          characterData2.characterId,
        ...
        if (realId) {
          ...
          const fingerprint = `${dicecloudUserId}-${characterData2.name}-${characterData2.class}-${characterData2.level}`;
          let fpHash = 0;
          for (let i = 0; i < fingerprint.length; i++) {
            const char = fingerprint.charCodeAt(i);
      ...
      const preparedRaw = prepareRawForPayload(characterData2);
      ...
      const payload = {
        ...
        death_saves: characterData2.deathSaves || { successes: 0, failures: 0 },
        inspiration: characterData2.inspiration || false,
        armor_class: extractArmorClass(preparedRaw, characterData2.armorClass || 10),
        speed: characterData2.speed || 30,
        initiative: characterData2.initiative || 0,
        proficiency_bonus: characterData2.proficiencyBonus || 2,
        attributes: characterData2.attributes || {},
        ...
        resources: characterData2.resources || [],
        conditions: characterData2.conditions || [],
        notification_color: characterData2.notificationColor || "#3498db",
      ...
      if (!response.ok) {
        ...
        const updateResponse = await fetch(
          `${window.SUPABASE_URL}/rest/v1/clouds_characters?dicecloud_character_id=eq.${characterData2.id}`,
          {
            method: "PATCH",
            headers: {
              "apikey": window.SUPABASE_ANON_KEY,
  ...
  async function checkDiscordCharacterIntegration(characterName, characterId) {
    try {
      ...
      const pairingResult = await browserAPI3.storage.local.get(["discordPairingId", "discordPairingCode"]);
      ...
      const characterData2 = await getCharacterData();
      const activeCharacterName = characterData2?.character_name || characterData2?.name;
      debug.log(`\u{1F50D} Character comparison: Discord="${characterName}" vs Active="${activeCharacterName}"`);
      debug.log(`\u{1F50D} Active character data:`, characterData2);
      ...
      if (characterData2 && normalizedDiscordName && normalizedActiveName && normalizedDiscordName === normalizedActiveName) {
        ...
        return {
          ...
          availableCharacter: characterData2 ? {
            name: activeCharacterName,
            race: characterData2.race,
            class: characterData2.class,
            level: characterData2.level
  ...
  function isSupabaseConfigured() {
    return window.window.SUPABASE_URL && !window.window.SUPABASE_URL.includes("your-project") && window.window.SUPABASE_ANON_KEY && window.window.SUPABASE_ANON_KEY !== "your-anon-key";
  }
  ...
  async function subscribeToCommandRealtime(pairingId) {
    if (!pairingId) {
      const settings = await browserAPI3.storage.local.get(["discordPairingId"]);
  ...
  async function executeCastCommand(command) {
    const { action_name, command_data } = command;
    try {
      const characterData2 = await getCharacterDataForDiscordCommand(command_data.character_name, command_data.character_id);
      if (!characterData2) {
  ...
  async function connectToInstaller() {
    try {
      ...
      installerPort = browserAPI3.runtime.connectNative("com.owlcloud.installer");
  ...
  async function broadcastToPopup(message) {
    try {
      await browserAPI3.runtime.sendMessage(message);
    } catch (error) {
