...
` + JSON.stringify(spellSlotProp, null, 2));
  ...
  (function() {
    ...
    browserAPI.runtime.onMessage.addListener((request, sender, sendResponse) => {
      ...
      if (request.action === "postRollToChat") {
        ...
        if (actionData.damageRoll || actionData.attackBonus) {
          const rollFormula = actionData.attackBonus ? `1d20+${actionData.attackBonus}` : actionData.damageRoll;
        ...
        if (spell.castingTime)
          announcement += ` {{Casting Time=${spell.castingTime}}}`;
        if (spell.range)
          announcement += ` {{Range=${spell.range}}}`;
        if (spell.duration)
          announcement += ` {{Duration=${spell.duration}}}`;
        if (spell.components)
          announcement += ` {{Components=${spell.components}}}`;
        if (spell.source)
          announcement += ` {{Source=${spell.source}}}`;
    ...
    `;
      ...
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px;">
        ...
        <div style="display: flex; gap: 8px;">
          <button id="import-players-btn" style="padding: 8px 14px; background: #27ae60; color: #fff; border: none; border-radius: 6px; cursor: pointer; font-size: 0.95em; font-weight: bold;">\u{1F4E5} Import</button>
    ...
    `;
      header.innerHTML = `
      <div>
        <h2 style="margin: 0; font-size: 1.2em; color: #4ECDC4;">\u{1F451} GM Panel</h2>
...
`;
    ...
    function toggleGMMode(enabled) {
      ...
      if (gmModeEnabled) {
        gmPanel.style.borderColor = "#4ECDC4";
      ...
      if (previousState !== gmModeEnabled) {
        const message = gmModeEnabled ? "\u{1F451} GM Panel is now active - rolls will be hidden from players" : "\u{1F451} GM Panel deactivated - rolls will post normally";
        setTimeout(() => {
