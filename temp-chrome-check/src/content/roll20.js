...
` + JSON.stringify(spellSlotProp, null, 2));
        ...
        const tryInitialize = async () => {
          try {
            ...
            if (typeof browserAPI !== "undefined" && browserAPI && browserAPI.storage && browserAPI.storage.local) {
              ...
              const result = await browserAPI.storage.local.get(["activeCharacterId", "characterProfiles", "carmaclouds_characters"]);
              const { activeCharacterId, characterProfiles, carmaclouds_characters } = result;
  ...
  var browserAPI2 = typeof chrome !== "undefined" ? chrome : typeof browser !== "undefined" ? browser : {};
  window.browserAPI = browserAPI2;
  (function() {
    "use strict";
    ...
    `;
      ...
      if (browserAPI2.storage.local.get instanceof Function) {
        browserAPI2.storage.local.get(["characterProfiles"]).then((result) => {
      ...
      try {
        browserAPI2.storage.local.get(["characterProfiles"], (result) => {
          debug.log("\u{1F9EA} Callback storage test result:", result);
          if (browserAPI2.runtime.lastError) {
            debug.error("\u{1F9EA} Callback storage error:", browserAPI2.runtime.lastError);
    ...
    function savePlayerDataToStorage() {
      ...
      return browserAPI2.storage.local.get(["characterProfiles"]).then((result) => {
        ...
        return new Promise((resolve, reject) => {
          browserAPI2.storage.local.set({
            characterProfiles: existingProfiles
          }, () => {
            if (browserAPI2.runtime.lastError) {
              debug.error("\u274C Error saving to storage:", browserAPI2.runtime.lastError);
              reject(browserAPI2.runtime.lastError);
            } else {
    ...
    window.showFullCharacterModal = function(playerName) {
      ...
      const popup = window.open(browserAPI2.runtime.getURL("src/popup-sheet.html"), `character-${playerName}`, "width=900,height=700,scrollbars=yes,resizable=yes,toolbar=no,menubar=no,location=no,status=no");
      if (!popup) {
...
`;
    ...
    async function markCharacterAsActive(characterName) {
      try {
        ...
        const result = await browserAPI2.storage.local.get(["characterProfiles"]);
        const characterProfiles = result.characterProfiles || {};
        ...
        if (characterId) {
          const response = await browserAPI2.runtime.sendMessage({
    ...
    if (typeof browserAPI2 !== "undefined" && browserAPI2.runtime) {
      browserAPI2.runtime.sendMessage({ action: "getManifest" }).then((response) => {
