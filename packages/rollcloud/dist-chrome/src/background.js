...
(() => {
      ...
      try {
        ...
        switch (request.action) {
          ...
          case "postChatMessageFromPopup": {
            debug.log("\u{1F4E8} Posting chat message from popup:", request.message?.substring(0, 100));
          ...
          case "ping":
            debug.log("\u{1F4E1} Received ping from content script");
        ...
        if (typeof SupabaseTokenManager !== "undefined") {
          ...
          if (currentDicecloudUserId) {
            const response = await fetch(
              `${supabase2.supabaseUrl}/rest/v1/rollcloud_characters?user_id_dicecloud=eq.${currentDicecloudUserId}&select=*`,
              {
                headers: {
  ...
  async function getCharacterDataFromDatabase(characterId) {
    try {
      ...
      const tryQuery = async (field, value) => {
        const url = `${supabase2.supabaseUrl}/rest/v1/rollcloud_characters?${field}=eq.${encodeURIComponent(value)}${userFilter}&select=*&order=updated_at.desc&limit=1`;
  ...
  async function setActiveCharacter(characterId) {
    try {
      ...
      try {
        ...
        if (tabs.length > 0) {
          const result2 = await browserAPI.storage.local.get(["characterProfiles"]);
          const characterProfiles2 = result2.characterProfiles || {};
          const characterData2 = characterProfiles2[characterId];
          if (characterData2 && characterData2.id) {
            debug.log(`Broadcasting active character change to Roll20 tabs: ${characterData2.id}`);
            for (const tab of tabs) {
              browserAPI.tabs.sendMessage(tab.id, {
                action: "activeCharacterChanged",
                characterId: characterData2.id,
                slotId: characterId
              }).catch((err) => {
  ...
  async function testDiscordWebhook(webhookUrl) {
    try {
      ...
      const testEmbed = {
        embeds: [{
          title: "\u{1F3B2} RollCloud Connected!",
    ...
    if (type === "actionUpdate") {
      ...
      return {
        embeds: [{
          ...
          color: hasUsedActions ? 15965202 : 16537593,
  ...
  async function executeRollHereCommand(command) {
    ...
    debug.log("\u{1F3B2} Rolling in Discord (not Roll20):", rollString2, rollName2);
    try {
      const payload = {
        type: "roll",
        characterName,
  ...
  async function executeRollCommand(command) {
    const { action_name, command_data } = command;
    const rollString2 = command_data.roll_string || `/roll 1d20`;
    const rollName2 = action_name || command_data.roll_name || "Discord Roll";
    const rollData = {
      formula: rollString2,
      name: rollName2,
      source: "discord",
      ...
      color: command_data.notification_color || "#3498db"
      // Include character's notification color for colored announcements
    };
    ...
    return { success: true, message: `Rolled ${rollName2}` };
  }
  ...
  async function executeCastCommand(command) {
    ...
    try {
      ...
      const castResult = typeof globalThis.executeDiscordCast === "function" ? globalThis.executeDiscordCast(command_data, characterData) : { rolls: [], effects: [], text: "" };
      debug.log("\u{1F52E} Discord spell execution result:", castResult);
  ...
  async function executeHealCommand(command) {
    ...
    try {
      ...
      return {
        success: true,
        message: command_data.is_temp ? `Added ${command_data.amount} temp HP` : `Healed ${command_data.amount} HP`
      };
    } catch (error) {
