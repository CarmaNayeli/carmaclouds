...
(() => {
  ...
  (function() {
    "use strict";
    debug.log("\u{1F3B2} RollCloud: DiceCloud content script loaded");
    ...
    function parseCharacterData(apiData) {
      ...
      try {
        ...
        debug.log(`\u{1F6E1}\uFE0F Properties with type/name containing armor/AC: ${propArmorMatches.length}`, propArmorMatches.map((p) => ({
          id: p._id || p.id || null,
          name: p.name,
          type: p.type,
          stat: p.stat,
      ...
      const calculateArmorClass = () => {
        ...
        try {
          ...
          if (creature && creature.denormalizedStats) {
            ...
            for (const k of tryKeys) {
              if (dn.hasOwnProperty(k)) {
                ...
                if (num !== null) {
                  debug.log(`\u{1F6E1}\uFE0F Using denormalizedStats.${k}:`, num);
                  return num;
                }
              }
            }
          ...
          if (creature && creature.armorClass !== void 0) {
            ...
            if (num !== null) {
              debug.log("\u{1F6E1}\uFE0F Using creature.armorClass:", num);
              return num;
            }
          }
          if (creature && creature.stats && creature.stats.ac !== void 0) {
            ...
            if (num !== null) {
              debug.log("\u{1F6E1}\uFE0F Using creature.stats.ac:", num);
              return num;
            }
          }
          try {
            if (variables && Object.keys(variables).length > 0) {
              ...
              for (const vn of varNamesToCheck) {
                if (Object.prototype.hasOwnProperty.call(variables, vn)) {
                  ...
                  if (candidate !== null) {
                    debug.log(`\u{1F6E1}\uFE0F Using variable ${vn}:`, candidate);
                    return candidate;
                  }
                }
              }
          ...
          try {
            if (Array.isArray(properties) && properties.length > 0) {
              ...
              if (propAC) {
                ...
                if (propNum !== null) {
                  debug.log('\u{1F6E1}\uFE0F Using property "Armor Class":', propNum, propAC);
                  return propNum;
                }
              }
            }
        ...
        try {
          ...
          if (creature && creature.denormalizedStats) {
            debug.log("\u{1F6E1}\uFE0F Deep-scanning denormalizedStats for numeric AC candidates...");
            debug.log("\u{1F6E1}\uFE0F denormalizedStats snapshot:", creature.denormalizedStats);
            ...
            if (namedFound) {
              debug.log(`\u{1F6E1}\uFE0F Found named numeric candidate in denormalizedStats at ${namedFound.path}:`, namedFound.value);
              return namedFound.value;
            }
            const found = findNumericRecursively(creature.denormalizedStats, 0, "denormalizedStats");
            if (found) {
              debug.log(`\u{1F6E1}\uFE0F Found numeric candidate in denormalizedStats at ${found.path}:`, found.value);
              return found.value;
            }
          }
        } catch (e) {
        ...
        try {
          ...
          if (armorVarKeys.length > 0) {
            debug.log("\u{1F6E1}\uFE0F armor variable candidates:", armorVarKeys);
            for (const key of armorVarKeys) {
              try {
                ...
                if (candidateNum !== null) {
                  debug.log(`\u{1F6E1}\uFE0F Using variable ${key}:`, candidateNum);
                  return candidateNum;
                } else {
                  debug.log(`\u{1F6E1}\uFE0F Variable ${key} exists but did not yield numeric AC:`, candidateVar);
                }
              } catch (inner) {
          ...
          for (const diceType of hitDiceTypes) {
            if (variables[diceType] && (variables[diceType].total ?? variables[diceType].max) > 0) {
              ...
              return {
                current: currentDice,
                max: maxDice,
                type: diceType.replace("HitDice", "").toLowerCase()
                // D8HitDice -> d8
              };
            }
          }
          ...
          case "spell":
            ...
            spellChildren.forEach((child) => {
              ...
              if (isDefensiveSpell && (child.type === "attack" || child.type === "roll" && child.name && child.name.toLowerCase().includes("attack"))) {
                debug.log(`    \u{1F6E1}\uFE0F Defensive spell - skipping attack child: ${child.name}`);
                return;
              }
            ...
            if (description) {
              ...
              if (!attackRoll && hasAttackMention && !isDefensiveSpell) {
                attackRoll = "use_spell_attack_bonus";
                debug.log(`  \u{1F4A1} Found offensive attack pattern in description, marking for spell attack bonus`);
              } else if (!attackRoll) {
                debug.log(`  \u26A0\uFE0F No attack pattern found in description for "${prop.name}"`);
              } else if (isDefensiveSpell && attackRoll) {
                debug.log(`  \u{1F6E1}\uFE0F Defensive spell detected - removing attack roll`);
                attackRoll = "";
              } else {
            ...
            if (isFinalDefensiveSpell && attackRoll) {
              debug.log(`  \u{1F6E1}\uFE0F Defensive spell detected - removing attack roll (was: "${attackRoll}")`);
              attackRoll = "";
            }
      ...
      if (characterData.resources && characterData.resources.length > 0) {
        debug.log(`\u{1F50D} Found ${characterData.resources.length} resources:`, characterData.resources.map((r) => r.name));
        const hpResource = characterData.resources.find(
          (r) => r.name.toLowerCase().includes("hit points") || r.name.toLowerCase() === "hp" || r.variableName?.toLowerCase() === "hitpoints"
        );
        const tempHpResource = characterData.resources.find(
        ...
        debug.log(`\u{1F50D} HP resource found: ${hpResource ? hpResource.name : "none"}`);
        debug.log(`\u{1F50D} Temp HP resource found: ${tempHpResource ? tempHpResource.name : "none"}`);
        if (hpResource) {
          characterData.hitPoints = {
            current: hpResource.current,
            max: hpResource.max
          };
          debug.log(`\u{1F49A} Mapped HP resource: ${hpResource.name} (${hpResource.current}/${hpResource.max})`);
        }
        if (tempHpResource) {
          characterData.temporaryHP = tempHpResource.current;
          debug.log(`\u{1F499} Mapped Temp HP resource: ${tempHpResource.name} (${tempHpResource.current})`);
        }
      } else {
        debug.log("\u{1F50D} No resources found in character data");
      }
...
`, item.element.textContent.substring(0, 200));
    ...
    function addSyncButton() {
      ...
      button.innerHTML = "\u{1F504} Sync to RollCloud";
    ...
    function syncCharacterData(slotId) {
      ...
      return extractCharacterData().then((characterData) => {
                ...
                browserAPI.storage.local.get(["characterProfiles"], (result) => {
                  ...
                  browserAPI.storage.local.set({ characterProfiles: profiles }, () => {
                    if (browserAPI.runtime.lastError) {
                      ...
                      showNotification(`\u2705 ${characterData.name} synced to RollCloud! \u{1F3B2}`, "success");
                      if (button) {
        ...
        if (error.message && error.message.includes("Not logged in")) {
          showNotification("\u26A0\uFE0F Please login to DiceCloud first! Click the RollCloud extension icon to login.", "error", 5e3);
    ...
    async function autoRefreshToken() {
      try {
        ...
        debug.log("\u{1F50D} Attempting direct localStorage access...");
    ...
    if (document.readyState === "loading") {
      document.addEventListener("DOMContentLoaded", () => {
        ...
        autoRefreshToken();
        setTimeout(() => {
          debug.log("\u{1F504} Retrying auto-refresh (2s delay)...");
          autoRefreshToken();
        }, 2e3);
        setTimeout(() => {
          debug.log("\u{1F504} Final auto-refresh attempt (5s delay)...");
          autoRefreshToken();
        }, 5e3);
      });
    } else {
