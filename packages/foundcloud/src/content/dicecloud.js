...
(function() {
  ...
  debug.log('🎲 FoundCloud: DiceCloud content script loaded');
    ...
    return extractCharacterData()
                ...
                browserAPI.storage.local.get(['characterProfiles'], (result) => {
                  ...
                  browserAPI.storage.local.set({ characterProfiles: profiles }, () => {
                    if (browserAPI.runtime.lastError) {
                      ...
                      showNotification(`✅ ${characterData.name} synced to FoundCloud! 🎲`, 'success');
                      if (button) {
      ...
      .catch(error => {
        ...
        if (error.message && error.message.includes('Not logged in')) {
          showNotification('⚠️ Please login to DiceCloud first! Click the FoundCloud extension icon to login.', 'error', 5000);
