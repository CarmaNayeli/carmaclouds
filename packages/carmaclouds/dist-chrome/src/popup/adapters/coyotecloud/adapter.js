...
${suffix}`;
  ...
  async function loadSyncedList(supabase, containerEl, dicecloudUserId) {
    ...
    if (!listEl || !dicecloudUserId)
      return;
    try {
      const { data, error } = await supabase.from("clouds_characters").select("dicecloud_character_id,character_name,level,class,race").eq("user_id_dicecloud", dicecloudUserId).order("updated_at", { ascending: false });
      if (error)
        throw error;
      listEl.innerHTML = "";
