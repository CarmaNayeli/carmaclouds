...
async function getActiveCharacter(discordUserId) {
  try {
    const response = await fetch(
      `${SUPABASE_URL}/rest/v1/clouds_characters?discord_user_id=eq.${discordUserId}&is_active=eq.true&select=*`,
      {
        headers: {
...
async function sendRollHereToExtension(interaction, rollData) {
  try {
    ...
    const pairingResponse = await fetch(
      `${SUPABASE_URL}/rest/v1/clouds_pairings?discord_user_id=eq.${interaction.user.id}&status=eq.connected&select=id`,
      {
        headers: {
          'apikey': SUPABASE_SERVICE_KEY,
          'Authorization': `Bearer ${SUPABASE_SERVICE_KEY}`
    ...
    const response = await fetch(`${SUPABASE_URL}/rest/v1/clouds_commands`, {
      method: 'POST',
      headers: {
