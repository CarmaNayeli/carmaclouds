...
async function deletePairingRecords(guildId, channelId) {
  ...
  const response = await fetch(
    `${SUPABASE_URL}/rest/v1/clouds_pairings?discord_guild_id=eq.${guildId}&discord_channel_id=eq.${channelId}`,
    {
      method: 'DELETE',
      headers: {
        'apikey': SUPABASE_SERVICE_KEY,
  ...
  if (!response.ok) {
    const errorText = await response.text();
    console.error('Supabase clouds_pairings DELETE error:', response.status, errorText);
