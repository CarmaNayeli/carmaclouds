...
async function pollForTurns() {
  ...
  try {
    ...
    const response = await fetch(
      `${SUPABASE_URL}/rest/v1/clouds_turns?status=eq.pending&order=created_at.asc&limit=10&select=*,clouds_pairings(*)`,
      {
        headers: {
...
async function postTurnToDiscord(turn) {
  const pairing = turn.clouds_pairings;
...
async function markTurnPosted(turnId, messageId, error = null) {
  ...
  try {
    ...
    await fetch(
      `${SUPABASE_URL}/rest/v1/clouds_turns?id=eq.${turnId}`,
      {
        method: 'PATCH',
        headers: {
